...
public:
  ControlToNrpn(t_symbol *sym, long ac, t_atom *av)
      : index_lsb(0), index_msb(0), value_lsb(0), value_msb(0), valid(false),
  ...
  void cc(long inlet, long cc_index, long cc_value) {
    ...
    case CC_DATA_MSB:
      // WARNING
      // Officially LSB is optional, but since Max provides us only separate
      // messages and not a buffer
      // we have no way of knowing if LSB will be provided or not. Apart from
      // buffering ourself but that
...
private:
  ...
  enum NrpnMode { kInvalid = -1, kTwosComplement = 0, k14bit };
